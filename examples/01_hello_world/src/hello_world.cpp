...
auto hello_world::update(const float dt) -> lge::result<> {
	...
	if(ctx.actions.get(hide_hello_world_action).pressed) {
		if(ctx.world.any_of<lge::hidden>(center_ent_)) {
			ctx.world.remove<lge::hidden>(center_ent_);
		} else {
			ctx.world.emplace<lge::hidden>(center_ent_);
		}
	}

	// toggle hidden on the world label, this will hide only the world label
	if(ctx.actions.get(hide_world_action).pressed) {
		if(ctx.world.any_of<lge::hidden>(world_text_ent_)) {
			ctx.world.remove<lge::hidden>(world_text_ent_);
		} else {
			ctx.world.emplace<lge::hidden>(world_text_ent_);
		}
	}
