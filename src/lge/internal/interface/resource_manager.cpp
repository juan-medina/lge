...
auto parse_json(const std::string_view uri) -> std::optional<jsoncons::json> {
...
auto resource_manager::load_sprite_sheet(const std::string_view uri) -> result<sprite_sheet_handle> {
	...
	const auto root = parse_json(uri);
	if(!root) [[unlikely]] {
	...
	sprite_sheets_.emplace(key, sprite_sheet_data{.texture = texture, .frames = parse_frames(*root)});
