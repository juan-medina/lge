...
TEST_CASE("bounds: center pivot produces centered quad", "[bounds]") {
	entt::registry world;
...
TEST_CASE("bounds: top_left pivot produces quad starting at origin", "[bounds]") {
	entt::registry world;
	auto system = make_system(world);
...
TEST_CASE("bounds: bottom_right pivot produces quad ending at origin", "[bounds]") {
	entt::registry world;
	auto system = make_system(world);

	const auto e =
		add_entity(world, lge::placement{0.F, 0.F, 0.F, {1.F, 1.F}, lge::pivot::bottom_right}, {100.F, 200.F});
...
TEST_CASE("bounds: entity without metrics does not get bounds", "[bounds]") {
	entt::registry world;
	auto system = make_system(world);
...
TEST_CASE("bounds: entity without placement does not get bounds", "[bounds]") {
	entt::registry world;
	auto system = make_system(world);
...
TEST_CASE("bounds: zero size produces degenerate quad at pivot", "[bounds]") {
	entt::registry world;
	auto system = make_system(world);
