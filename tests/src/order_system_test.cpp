...
TEST_CASE("order: root entity gets its own layer and index", "[order]") {
	entt::registry world;
...
TEST_CASE("order: entity without order does not get render_order", "[order]") {
	entt::registry world;
	auto system = make_system(world);
...
TEST_CASE("order: child inherits parent layer", "[order][hierarchy]") {
	entt::registry world;
	auto system = make_system(world);
...
TEST_CASE("order: child index adds to parent index", "[order][hierarchy]") {
	entt::registry world;
	auto system = make_system(world);
...
TEST_CASE("order: child without order component gets parent index", "[order][hierarchy]") {
	entt::registry world;
	auto system = make_system(world);
...
TEST_CASE("order: grandchild inherits accumulated index", "[order][hierarchy][deep]") {
	entt::registry world;
	auto system = make_system(world);
